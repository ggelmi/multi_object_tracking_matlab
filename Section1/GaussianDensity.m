    ...
    methods (Static)
        ...
        function predicted_likelihood = predictedLikelihood(state_pred,z,measmodel)
            ...
            %Measurement model Jacobian
                   Hx = measmodel.H(state_pred.x);
                    ...
                    zbar = measmodel.h(state_pred.x);
                    
                     predicted_likelihood= log_mvnpdf(z',zbar',S);
        end
        ...
        function [z_ingate, meas_in_gate] = ellipsoidalGating(state_pred, z, measmodel, gating_size)
            ...
            %        (number of measurements) x 1
                      N = size(z,2); % Number of measurements
                      m = size(z,1);  % Measurement dimension
                      n = size(state_pred,1);
                      
                     meas_in_gate = zeros(N,1);
                    ...
                    S = (S+S')/2; 
                    zbar = measmodel.h(state_pred.x);
                    meas_in_gate = meas_in_gate | diag((z-zbar)'*inv(S)*(z-zbar)) < gating_size;
                    end
        ...
        function state = momentMatching(w, states)
            ...
            w = exp(w);
            % dimensions of state
            Nstate = size(states(1).x,1);
            % Number of Gaussians
            NumG = size(w,1);
            state.x = zeros(Nstate,1);
            state.P = zeros(Nstate,Nstate);
            for i = 1:NumG
